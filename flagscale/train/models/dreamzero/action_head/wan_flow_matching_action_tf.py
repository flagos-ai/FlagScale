...
from flagscale.train.models.dreamzero.n1_5.action_head.base_action_head import ActionHead
from flagscale.train.models.dreamzero.modules.flow_match_scheduler import FlowMatchScheduler
from flagscale.train.models.dreamzero.modules.vram_management import enable_vram_management, AutoWrappedModule, AutoWrappedLinear
from flagscale.train.models.dreamzero.modules.wan_video_text_encoder import T5RelativeEmbedding, T5LayerNorm
from flagscale.train.models.dreamzero.modules.flow_unipc_multistep_scheduler import FlowUniPCMultistepScheduler
...
class WANPolicyHead(ActionHead):
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        if videos.dtype == torch.uint8:
            ...
            # Match reference precision: VAE encoding runs in fp32 (reference has all params
            # in fp32 due to add_lora_to_model casting, so self.dtype=fp32 there).
            # Under FSDP2, self.dtype=bf16, but we force fp32 here for alignment.
        ...
        # AGENT_DEBUG: Fix random state for Level 4 alignment comparison
        import os as _os_align  # AGENT_DEBUG
        if _os_align.environ.get("DREAMZERO_ALIGN_SEED"):  # AGENT_DEBUG
            _align_seed = int(_os_align.environ["DREAMZERO_ALIGN_SEED"])  # AGENT_DEBUG
            torch.manual_seed(_align_seed)  # AGENT_DEBUG
            torch.cuda.manual_seed(_align_seed)  # AGENT_DEBUG
        # END AGENT_DEBUG
        ...
        if actions.numel() > 0:
            ...
            # NOTE: These assertions are invalid for the DreamZero-AgiBot checkpoint config
            # (num_frames=33, num_frame_per_block=2, num_action_per_block=48, action_horizon=48)
            # 48/8=6 != 48//2=24. Commented out for training.
    ...
    def post_initialize(self):
        ...
        if LOAD_TRT_ENGINE is not None:
            ...
            import flagscale.train.models.dreamzero.modules.tensorrt_utils as trt_utils
