...
from flagscale.train.models.dreamzero.modules.utils import hash_state_dict_keys
from flagscale.train.models.dreamzero.modules.wan_video_camera_controller import SimpleAdapter
try:
...
@torch.compile(disable=DISABLE_TORCH_COMPILE)
def rope_apply_no_polar_op(x, freqs_cis, num_heads):
    freqs_cos, freqs_sin = freqs_cis

    x_reshaped = rearrange(x, "b s (h d) -> b s h d", h=num_heads)
    x_as_complex = x_reshaped.float().reshape(*x_reshaped.shape[:-1], -1, 2)
    x_real, x_imag = x_as_complex.unbind(-1)

    freqs_cos = freqs_cos.to(x.device).squeeze(1).unsqueeze(0).unsqueeze(2)
    freqs_sin = freqs_sin.to(x.device).squeeze(1).unsqueeze(0).unsqueeze(2)
    ...
    return rearrange(x_out, "b s h d -> b s (h d)").to(x.dtype)


@torch.compile(disable=DISABLE_TORCH_COMPILE)
def rope_apply_polar_op(x, freqs, num_heads):
    x = rearrange(x, "b s (n d) -> b s n d", n=num_heads)
    x_out = torch.view_as_complex(x.to(torch.float64).reshape(
        x.shape[0], x.shape[1], x.shape[2], -1, 2))
    freqs = freqs.to(x_out.device)
...
def _RMSNorm(normalized_shape, eps):
...
class RotaryPositionEmbeddingNoPolarOp(nn.Module):
    def __init__(self, num_heads: int, head_dim: int):
        ...
        self.freqs = self.precompute_freqs_cis_3d(head_dim)

    def precompute_freqs_cis_3d(self, dim: int, end: int = 1024, theta: float = 10000.0):
        # 3d rope precompute
    ...
    def precompute_freqs_cis(self, dim: int, end: int = 1024, theta: float = 10000.0):
        # 1d rope precompute
    ...
    def forward(self, f: int, h: int, w: int, a: int) -> torch.Tensor:
        freqs_cos_3d = torch.cat(
            [
                self.freqs["f"][0][:f].view(f, 1, 1, -1).expand(f, h, w, -1),
                self.freqs["h"][0][:h].view(1, h, 1, -1).expand(f, h, w, -1),
                self.freqs["w"][0][:w].view(1, 1, w, -1).expand(f, h, w, -1),
            ],
            dim=-1,
        ).reshape(f * h * w, 1, -1)
        freqs_sin_3d = torch.cat(
            [
                self.freqs["f"][1][:f].view(f, 1, 1, -1).expand(f, h, w, -1),
                self.freqs["h"][1][:h].view(1, h, 1, -1).expand(f, h, w, -1),
                self.freqs["w"][1][:w].view(1, 1, w, -1).expand(f, h, w, -1),
            ],
            dim=-1,
        ).reshape(f * h * w, 1, -1)
...
class AttentionModule(nn.Module):
    ...
    def forward(self, q, k, v):
        x = flash_attention(q=q, k=k, v=v, num_heads=self.num_heads, compatibility_mode=FLASH_ATTN_COMPATIBILITY_MODE)
        return x
...
class SelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6):
        ...
        self.norm_q = _RMSNorm(dim, eps=eps)
        self.norm_k = _RMSNorm(dim, eps=eps)
...
class CrossAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6, has_image_input: bool = False):
        ...
        if has_image_input:
            ...
            self.norm_k_img = _RMSNorm(dim, eps=eps)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        if self.has_image_input:
            ...
            y = flash_attention(q, k_img, v_img, num_heads=self.num_heads, compatibility_mode=FLASH_ATTN_COMPATIBILITY_MODE)
            x = x + y
        return self.o(x)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs):
        ...
        input_x = modulate(self.norm1(x), shift_msa, scale_msa).to(dtype=t_mod.dtype)
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs))
        x = x + self.cross_attn(self.norm3(x).to(dtype=t_mod.dtype), context.to(dtype=t_mod.dtype))
...
class Head(nn.Module):
    ...
    def forward(self, x, t_mod):
        ...
        shift, scale = (self.modulation.repeat(batch_size,1,1).to(dtype=t_mod.dtype, device=t_mod.device) + t_mod.unsqueeze(1)).chunk(2, dim=1)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.rope = RotaryPositionEmbedding(num_heads=num_heads, head_dim=head_dim)
    ...
    def forward(self,
        ...
        # print("x before patchify", x.shape)
        # clip embedding shape of B * 257 * dim (5120)
        # context shape of B * 769 * dim (5120) - 257 + 512
        # x before patchify B * 36 * l_t * l_h * l_w 
        x, (f, h, w) = self.patchify(x)
        # x after patchify B * 512 * 5120
        # f = 2, h = 16, w = 16
        ...
        x = self.head(x, t)
        # print("x before unpatchify", x.shape)
        x = self.unpatchify(x, (f, h, w))
        # print("x after patchify", x.shape)
        return x
    def post_initialize(self):
        self.rope.post_initialize()
    @staticmethod
