...
from flagscale.train.models.dreamzero.modules.wan2_1_attention import AttentionModule
from flagscale.train.models.dreamzero.n1_5.modules.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)
from flagscale.train.models.dreamzero.modules.wan2_1_submodule import (
    WanRMSNorm,
    rope_action_apply,
    WanLayerNorm,
...
class CausalWanSelfAttention(nn.Module):
    ...
    def _process_noisy_image_blocks(self, noisy_image_q, noisy_image_k, noisy_image_v,
        ...
        Matches reference behavior: per-block slicing of action/state tokens.
        With action_horizon=24 and num_action_per_block=24, only block 0 gets
        action/state context (blocks 1+ slice beyond tensor bounds → empty).

        First frame: conditioning, self-attention only.
        Block i: attends to first_clean_frame + clean_blocks[0:i] + current_noisy_block
                 + action[i*num_action_per_block:(i+1)*num_action_per_block]
                 + state[i*num_state_per_block:(i+1)*num_state_per_block]
        """
        block_size = self.frame_seqlen * self.num_frame_per_block
        ...
        # Pre-compute all block indices (matches reference)
        ...
        for block_idx in range(num_blocks):
            ...
            # Build context: first_clean_frame + clean_blocks[0:i] + current_noisy_block
            #                + action[i] + state[i]
            # NOTE: For blocks > 0, action/state slices are beyond tensor bounds
            # (empty tensors) — this matches reference behavior exactly.
            k_context = torch.cat([
    ...
    def _process_noisy_action_blocks(self, noisy_action_q, noisy_action_k, noisy_action_v,
        ...
        Matches reference behavior: per-block slicing.
        Action block i: attends to first_clean_frame + clean_blocks[0:i]
                        + noisy_image[i] + action[i] + state[i]

        With action_horizon=24 and num_action_per_block=24, only block 0 has
        actual tokens; blocks 1+ slice beyond bounds → empty queries, producing
        empty output (no contribution to final sequence).
        """
        num_blocks = (half_frames - 1) // self.num_frame_per_block
        ...
        for block_idx in range(num_blocks):
            ...
            # Skip if this block has no query tokens (OOB slice → empty)
            if q_block.shape[1] == 0:
                continue

            # Build context: first_clean_frame + clean_blocks[0:i] + noisy_image[i]
            #                + action[i] + state[i]
            k_context = torch.cat([
                clean_image_k[:, :clean_end],
        ...
        if kv_cache is None:
            if is_tf:
                ...
                if action_register_length is not None:
                    ...
                    # action_register_length is the TOTAL register tokens appended:
                    # = num_action_tokens + num_state_tokens
                    # Per-block slicing means only block 0 gets actual tokens;
                    # blocks 1+ slice beyond bounds → empty (matches reference).
                    action_horizon = action_register_length - self.num_state_per_block
                    state_horizon = self.num_state_per_block

                    # Validate sequence length matches layout
                    expected_total = half_seq_len + noisy_image_seq_len + action_horizon + state_horizon
                    if roped_query.shape[1] != expected_total:
                        raise ValueError(
                            f"Sequence length does not match block layout. "
                            f"action_register_length={action_register_length}, "
                            f"action_horizon={action_horizon}, state_horizon={state_horizon}. "
                            f"Expected total={expected_total}, got={roped_query.shape[1]}."
                        )
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_train(
        ...
        # arguments
        context_casted = context.to(dtype=x.dtype) if context is not None else context
        kwargs = dict(
            e=e0,
            freqs=freqs,
            freqs_action=self.freqs_action,
        ...
        for block in self.blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                if ckpt_use_reentrant:
                    # use_reentrant=True requires positional args only (no kwargs).
                    # Faster than non-reentrant (no tensor pack/unpack hooks).
