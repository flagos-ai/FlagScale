...
def on_save_checkpoint_start(async_save):
    ...
    if one_logger:
        with one_logger.get_context_manager():
            ...
            if async_save:
                productive_metrics.update({'save_checkpoint_async_count': save_checkpoint_count})
...
def track_app_tag(batch_size, world_size, seq_length):
    ...
    if one_logger:
        with one_logger.get_context_manager():
            ...
            current_app_tag = (
                f'{app_tag_run_name}_{app_tag_run_version}_{batch_size}'
