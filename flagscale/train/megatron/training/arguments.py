...
def load_retro_args(args):
    ...
    args.merge_file = (
        os.path.join(args.retro_project_dir, retro_config.retro_gpt_merge_file)
    ...
    args.tokenizer_model = (
        os.path.join(args.retro_project_dir, retro_config.retro_gpt_tokenizer_model)
    ...
    args.vocab_file = (
        os.path.join(args.retro_project_dir, retro_config.retro_gpt_vocab_file)
...
def validate_args(args, defaults={}):
    ...
    if not enable_hetero:
        ...
        if args.perform_rl_step:
            assert not (
                args.rl_remove_kv_cache_during_training and args.rl_offload_kv_cache_during_training
            ), "Cannot use both remove-kv-cache-during-training and offload-kv-cache-during-training"

            assert not (
                args.rl_partial_rollouts and args.rl_remove_kv_cache_during_training
    ...
    if args.overlap_param_gather:
        assert (
            args.use_distributed_optimizer or args.use_megatron_fsdp
    ...
    if args.retro_add_retriever:
        ...
        assert (
            args.train_samples is not None
    ...
    if args.inference_dynamic_batching:
        ...
        assert (
            args.inference_dynamic_batching_block_size % 256 == 0
...
def _add_inference_args(parser):
    ...
    group.add_argument(
        "--cuda-graph-warmup-steps", type=int, default=3, help="Number of CUDA graph warmup steps"
    )
    group.add_argument(
        '--external-cuda-graph',
        action='store_true',
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--hidden-size', type=int, default=None, help='Transformer hidden size.')
    group.add_argument(
        '--ffn-hidden-size',
        type=int,
        default=None,
...
def _add_straggler_detector_args(parser):
    ...
    group.add_argument(
        '--log-straggler', action='store_true', help='If set, tracks and logs straggler per GPU.'
    )
    group.add_argument(
...
def _add_logging_args(parser):
    ...
    group.add_argument(
        '--log-energy', action='store_true', help='If set, log energy consumption (in Joules)'
    )
    group.add_argument(
    ...
    group.add_argument('--logging-level', type=int, default=None, help='Set default logging level')
    return parser
...
def _add_rl_args(parser):
    ...
    group.add_argument(
        '--grpo-default-temperature',
        type=float,
        default=1.0,
        help="Default temperature for model inference.",
    )
    group.add_argument(
        '--grpo-default-top-p', type=float, default=0, help="Default top-p for model inference."
    )
    group.add_argument(
        '--langrl-inference-server-type',
        type=str,
    ...
    group.add_argument(
        '--langrl-inference-server-conversation-template',
        type=str,
        default=None,
        help="Conversation template, if using a chat server.",
    )
    group.add_argument(
        '--langrl-external-server',
    ...
    group.add_argument(
        '--rl-use-sequence-packing', action='store_true', help='Enable sequence packing'
    )
    group.add_argument(
        '--rl-sequence-packing-bin-size',
        type=int,
        default=8192,
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument(
        '--ckpt-step', type=int, default=None, help='Checkpoint step to load model from.'
    )
    group.add_argument(
        '--no-initialization',
        action='store_false',
    ...
    group.add_argument(
        '--ckpt-convert-save', default=None, help='Save directory for converted checkpoint.'
    )
    group.add_argument(
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        ...
        action='store_true',
        default=False,
        help='If set, defers the vocabulary projection linear layer weight'
        'gradient compute to pipeline flush.',
        dest='defer_embedding_wgrad_compute',
    )
    group.add_argument(
        '--wgrad-deferral-limit',
        type=int,
        default=0,
        help='Number of micro-batches for which'
    ...
    group.add_argument(
        ...
        action='store_true',
        default=False,
        help='If set, *input* embedding layer will be treated as a standard transformer'
    ...
    group.add_argument(
        ...
        action='store_true',
        default=False,
        help='If set, loss layer will be treated as a standard transformer'
        'layer in the context of partition and placement for pipeline parallelism.',
    )
    group.add_argument(
    ...
    group.add_argument(
        '--use-sharp', action='store_true', help='Required to enable SHARP communication.'
    )
    group.add_argument(
        '--sharp-enabled-group',
        type=str,
    ...
    group.add_argument(
        '--use-megatron-fsdp', action='store_true', help='Use the Megatron FSDP code path in DDP.'
    )
...
def _add_validation_args(parser):
    ...
    group.add_argument(
        "--test-mode", action="store_true", help='Run all real-time test alongside the experiment.'
    )
    group.add_argument(
        '--skip-train',
        action='store_true',
...
def _add_vision_args(parser):
    ...
    group.add_argument('--qk-l2-norm', action='store_true', help='Use llama 4 qk l2 norm')
...
def _add_moe_args(parser):
    ...
    group.add_argument(
        '--moe-hybridep-num-sms', type=int, default=16, help='Number of SMs to use for HybridEP.'
    )
    group.add_argument(
        '--moe-permute-fusion',
        action='store_true',
...
def _add_experimental_args(parser):
    ...
    group.add_argument(
        '--mamba-state-dim', type=int, default=128, help='State dimension for Mamba layers.'
    )
    group.add_argument(
        '--mamba-head-dim', type=int, default=64, help='Head dimension for Mamba layers.'
    )
    group.add_argument(
        '--mamba-num-groups', type=int, default=8, help='Number of groups for Mamba layers.'
    )
    group.add_argument(
        '--mamba-num-heads',
        type=int,
