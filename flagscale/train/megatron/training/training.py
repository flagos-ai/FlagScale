...
def num_floating_point_operations(
    ...
    def transformer_flops():
        ...
        dsv4_hybrid_standard_self_attn_term = 0  # FlagScale Modify
        ...
        self_attn_term = (
            ...
            + dsv4_hybrid_standard_self_attn_term  # FlagScale Modify
        )
...
def pretrain(
    ...
    extra_valid_dataset_provider=None,  # FlagScale Modify
):
    ...
    if _STARTUP_TIMESTAMPS['program_start'] is not None:
        program_start_global = torch.tensor([_STARTUP_TIMESTAMPS['program_start']], dtype=torch.double, device=cur_platform.device())  # FlagScale Modify
    ...
    if args.perform_rl_step:
        if (
            ...
            if use_torch_saver_for_inference_model:
                ...
                model_alloc_ctx = cur_platform.use_mem_pool(uvm_mempool)  # FlagScale Modify
            else:
    ...
    if not cfg_container.validation.skip_train or args.perform_rl_step:
        ...
        if args.do_train and (args.train_iters or 0) > 0:
            iteration, num_floating_point_operations_so_far = train(
                ...
                non_loss_data_func,
                inference_model,
                extra_valid_dataset_provider,  # FlagScale Modify
            )
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True, config=None, pg_collection=None):
    ...
    if (
        ...
        for model_module in model:
            model_module.to(cur_platform.current_device())  # FlagScale Modify
    ...
    if args.init_model_with_meta_device and not args.use_torch_fsdp2 and not args.use_megatron_fsdp:
        model = [to_empty_if_meta_device(model_module, device=torch.device(cur_platform.device_name())) for model_module in model]  # FlagScale Modify
    ...
    if wrap_with_ddp:
        ...
        ddp_stream = cur_platform.Stream()  # FlagScale Modify
        # Wait for the default stream to complete before starting ddp_stream
        ddp_stream.wait_stream(cur_platform.current_stream())  # FlagScale Modify
        # Make ddp_stream start after whatever the default stream already queued
        with cur_platform.stream(ddp_stream):  # FlagScale Modify
        ...
        cur_platform.current_stream().wait_stream(ddp_stream)  # FlagScale Modify
...
def get_optimizer_param_scheduler(optimizer):
    ...
    opt_param_scheduler = OptimizerParamScheduler(
        ...
        stablelm2_scheduler_config=stablelm2_scheduler_config,  # FlagScale Modify
    )
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func, iteration=None):
    ...
    if args.empty_unused_memory_level >= 1:
        cur_platform.empty_cache()  # FlagScale
...
def training_log(
    ...
    for key in loss_dict:
        if not skipped_iter:
            total_loss_dict[key] = (
                total_loss_dict.get(key, torch.tensor([0.0], dtype=torch.float, device=cur_platform.device_name()))  # FlagScale Modify
                + loss_dict[key]
            )
        else:
    ...
    # Tensorboard values.
    if is_last_rank() and (iteration % args.tensorboard_log_interval == 0):  # FlagScale Modify
        if wandb_writer:
            wandb_writer.log({'samples vs steps': args.consumed_train_samples}, iteration)
        if learning_rate is not None:
            if writer:  # FlagScale Modify
                writer.add_scalar('learning-rate', learning_rate, iteration)
                writer.add_scalar('learning-rate vs samples', learning_rate, args.consumed_train_samples)
            if wandb_writer:
                wandb_writer.log({'learning-rate': learning_rate}, iteration)
                wandb_writer.log({'consumed-tokens': args.consumed_train_samples * args.seq_length / 1000. / 1000 / 1000}, iteration)  # FlagScale Modify
        ...
        if args.log_memory_to_tensorboard:
            mem_stats = cur_platform.memory_stats()  # FlagScale Modify
            if writer:  # FlagScale Modify
                writer.add_scalar(
    ...
    if iteration % args.log_interval == 0 or is_first_iteration:
        ...
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key, nan_iters_key]:
                ...
                if should_reset:
                    total_loss_dict[key] = torch.tensor([0.0], dtype=torch.float, device=cur_platform.device_name())  # FlagScale Modify
...
def save_checkpoint_and_time(
    ...
    if args.log_energy:  # FlagScale Modify
...
def checkpoint_and_decide_exit(
    ...
    if args.exit_duration_in_mins:
        ...
        done_cuda = torch.tensor(
            [train_time > args.exit_duration_in_mins], dtype=torch.int, device=cur_platform.device_name()  # FlagScale Modify
        )
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        if get_num_microbatches() != num_microbatches and iteration != 0 \
            and args.save_when_num_microbatches_change:  # FlagScale Modify
    ...
    writer = get_tensorboard_writer()
    perf_monitor_end_training(writer, wandb_writer)  # FlagScale Modify
    if writer:
        writer.flush()
...
def evaluate(
    ...
    non_loss_data_func=None,
    eval_iters=None,
    extra_valid_index=None,  # FlagScale Modify
):
    """Evaluation."""
    args = get_args()
    timers = get_timers()
    ...
    with torch.no_grad():
        ...
        while iteration < eval_iters:
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                ...
                for key in loss_dicts[0].keys():
                    if key not in total_loss_dict:
                        total_loss_dict[key] = torch.tensor([0.0, 0.0], dtype=torch.float, device=cur_platform.device())  # FlagScale Modify
            ...
            if args.exit_duration_in_mins:
                train_time = (time.time() - _TRAIN_START_TIME) / 60.0
                done_cuda = torch.tensor(
                    [train_time > args.exit_duration_in_mins], dtype=torch.int, device=cur_platform.device()  # FlagScale Modify
                )
                torch.distributed.all_reduce(done_cuda, op=torch.distributed.ReduceOp.MAX)
                done = done_cuda.item()
                if done:
...
def evaluate_and_print_results(
    ...
    if args.full_validation:
        ...
        if mpu.get_tensor_model_parallel_rank() == 0:
            eval_iters = torch.tensor(args.eval_iters, dtype=torch.long, device=cur_platform.device_name())  # FlagScale Modify
        else:
            eval_iters = torch.tensor([0] * len(eval_iters), dtype=torch.long, device=cur_platform.device_name())  # FlagScale Modify
...
def build_train_valid_test_data_loaders(build_train_valid_test_datasets_provider):
    ...
    if is_distributed or mpu.get_tensor_model_parallel_rank() == 0:
        ...
        flags = torch.tensor(
            [int(do_train), int(do_valid), int(do_test)], dtype=torch.long, device=cur_platform.device_name()  # FlagScale Modify
        )
    else:
        flags = torch.tensor([0, 0, 0], dtype=torch.long, device=cur_platform.device_name())  # FlagScale Modify
