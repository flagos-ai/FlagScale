...
class EagleBackbone(nn.Module):
    ...
    def fsdp_units(self) -> list[nn.Module]:
        ...
        if hasattr(self.eagle_model, "vision_model"):
            ...
            if hasattr(vm, "vision_model"):
                vm = vm.vision_model
            units.extend(list(vm.encoder.layers))
        if hasattr(self.eagle_model, "language_model"):
            units.extend(list(self.eagle_model.language_model.model.layers))
        return units


class FlowmatchingActionHead(nn.Module):
    """Multi-embodiment flow matching action head for GR00T N1.5.

    Uses CategorySpecificMLP / MultiEmbodimentActionEncoder for per-embodiment
    encoding, and SelfAttentionTransformer for VL self-attention before
    cross-attention in the DiT.
    """
    ...
    def fsdp_units(self) -> list[nn.Module]:
        return list(self.model.transformer_blocks)
...
class GR00TN15(PreTrainedModel):
    ...
    def validate_inputs(self, inputs):
        ...
        if "video" in inputs:
            video = inputs["video"]
            type_ok = isinstance(video, __import__("numpy").ndarray)
            dtype_ok = video.dtype == __import__("numpy").uint8
    ...
    def prepare_input(self, inputs) -> tuple[BatchFeature, BatchFeature]:
        ...
        if tree is not None:
            ...
            backbone_inputs = BatchFeature(
                data={k: to_device_with_maybe_dtype(v) for k, v in backbone_inputs.items()}
            )
            action_inputs = BatchFeature(
                data={k: to_device_with_maybe_dtype(v) for k, v in action_inputs.items()}
            )
