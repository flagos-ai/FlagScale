...
class EngramMoule(nn.Module):
    def __init__(self, engram_cfg: Union[DeepSeekConfig, TransformerConfig], layer_id):
        super().__init__()
    ...
    def forward(self, hidden_states, hash_input_ids):
        ...
        hidden_states = hidden_states.view(seq_len, batch_size, self.backbone_config.num_residual_streams, -1)
        if self.embedding_cache is not None:
            ...
            assert hash_input_ids is not None, "If there is no embedding cache, hash input ids can not be None for Engram"
