...
@triton.jit
def _attn_fwd_search_1(Q, K, V, sm_scale, M, Out, Block_avg, Cached_lse, #
    ...
    cached_lse_ptr = Cached_lse + off_hz * N_CTX + offs_m
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        qk = tl.dot(q, k)
        cached_lse_block = tl.load(cached_lse_ptr)
        qk = qk * qk_scale
        p = tl.exp2(qk - cached_lse_block[:, None])
...
@triton.jit
def _attn_fwd_search_2(Q, K, V, sm_scale, M, Out, Block_avg,Cached_lse, #
              stride_qz, stride_qh, stride_qm, stride_qk,  #
              stride_kz, stride_kh, stride_kn, stride_kk,  #
              stride_vz, stride_vh, stride_vk, stride_vn,  #
              stride_oz, stride_oh, stride_om, stride_on,  #
    ...
    # load q: it will stay in SRAM throughout
    q = tl.load(Q_block_ptr)
    cached_lse_ptrs = Cached_lse + off_hz * N_CTX + offs_m
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        k = tl.load(K_block_ptr)
        qk = tl.dot(q, k)
        cached_lse_block = tl.load(cached_lse_ptrs)
        qk = qk * qk_scale
        p2 = tl.exp2(qk - cached_lse_block[:, None])
...
def _flash_attn_triton_search_cache_lse(q, k, v, cached_lse, causal = False, sm_scale = None, BLOCK_M = 128, BLOCK_N = 128, sparse_block_size = 128,):
        ### q.shape = [B, H, C, D]
        # shape constraints
        ...
        if BLOCK_M == BLOCK_N and BLOCK_M == sparse_block_size: 
            _attn_fwd_search_1[grid](
                q, k, v, sm_scale, M, o, block_avg, cached_lse,#
