...
def get_dataset_ids(base_url: str, secret: str, dataset_keys: list[str]) -> list[int]:
    ...
    key_to_id = {item["key"]: item["id"] for item in data}
    missing = [k for k in dataset_keys if k not in key_to_id]
    if missing:
        logger.info(f"Available dataset keys: {sorted(key_to_id)}")
        raise ValueError(f"Dataset keys not found: {missing}")
    return [key_to_id[k] for k in dataset_keys]
...
def get_archive_url(base_url: str, secret: str, batch_id: int, poll_interval: int) -> None:
    url = f"{base_url}/robo-batches/{batch_id}/archive"
    while True:
        headers = make_headers(url, "", secret)
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
        data = resp.json()
        if data.get("done"):
            logger.info(f"Download link: {data['url']}")
            return
        logger.info("Archive not ready yet, polling...")
        time.sleep(poll_interval)
...
def main():
    ...
    try:
        ...
        batch_id = submit_eval(
            args.base_url, secret, args.model_name, dataset_ids, ws_url, args.description
        )
        ...
        get_archive_url(args.base_url, secret, batch_id, args.poll_interval)
