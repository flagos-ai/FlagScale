...
def find_latest_stdout_log(start_path):
    ...
    attempt_dirs.sort(key=lambda x: int(x.split("_")[1]) if x.split("_")[1].isdigit() else -1, reverse=True)
...
def test_train_equal(test_path, test_type, test_task, test_case):
    ...
    for key in metric_keys:
        ...
        if len(result_values) != len(gold_values):
            print(f"WARNING: Length mismatch for '{key}': got {len(result_values)}, expected {len(gold_values)}")
            # Try to compare what we have
            min_len = min(len(result_values), len(gold_values))
            if min_len > 0:
                is_close = np.allclose(gold_values[:min_len], result_values[:min_len], rtol=1e-3, atol=1e-5)
