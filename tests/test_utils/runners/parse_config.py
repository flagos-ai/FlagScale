...
def load_yaml(path):
    ...
    if not os.path.isfile(path):
        raise IOError(f"Configuration file not found: {path}")
    try:
        with open(path) as f:
...
def get_platform_config(platform="default"):
    ...
    if not os.path.exists(config_file):
        raise IOError(f"Platform config not found: {config_file}")
...
def get_platform_data(config, platform="default"):
    ...
    if platform_key not in config:
        raise ValueError(f"Platform '{platform_key}' not found in config. Available: {list(config.keys())}")
    ...
    if task:
        ...
        if model:
            if model not in task_data:
                raise ValueError(f"Model '{model}' not found in task '{task}'. Available: {list(task_data.keys())}")
