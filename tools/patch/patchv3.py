...
def patch_hardware(main_path, commit, backends, device_type, tasks, key_path=None):
    ...
    # ============================ MODIFICATION 1 =================================
    patch_info = prompt_info(main_path, backends, device_type, tasks, flagscale_base_commit=commit)
    generate_patch_file(main_path, commit, patch_info, key_path=key_path)


def prompt_info(main_path, backends, device_type, tasks, flagscale_base_commit=None):
    ...
    # ============================== MODIFICATION 2 =================================
    logger.info(f"2. Resolving backend commits from FlagScale base commit: {flagscale_base_commit}...")
    
    main_repo = Repo(main_path)
    if not flagscale_base_commit:
        raise ValueError("FlagScale base commit is required to resolve submodule versions.")
    
    try:
        base_commit_obj = main_repo.commit(flagscale_base_commit)
    except Exception:
        raise ValueError(f"Commit {flagscale_base_commit} not found in FlagScale repository.")
    ...
    for backend in backends:
        ...
        try:
            original_submodule_commit = base_commit_obj.tree[submodule_path].hexsha
            backends_commit[backend] = original_submodule_commit
            logger.info(f"    -> {backend}: Auto-detected official commit {original_submodule_commit}")
        except KeyError:
             logger.error(f"    -> {backend}: Submodule path not found in commit {flagscale_base_commit}")
             backends_commit[backend] = None
        except Exception as e:
            logger.error(f"    -> {backend}: Failed to resolve commit. Error: {e}")
            backends_commit[backend] = None
    # ============================ MODIFICATION 2 END ===============================
    ...
    try:
        ...
        for backend in backends:
            ...
            if backend != FLAGSCALE_BACKEND:
                ...
                # ============================ MODIFICATION 3: ==========================
                submodule_commit_in_fs = submodule.module().head.commit.hexsha
