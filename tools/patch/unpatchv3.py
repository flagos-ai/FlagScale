        ...
        if fs_extension:
            ...
            try:
                ...
                if sub_repo.is_dirty(untracked_files=True):
                    logger.info(f"Committing FlagScale patches to submodule {submodule_name}...")

                    sub_repo.git.add(all=True)
                    sub_repo.git.commit('-m', f'Apply FlagScale base patches for {submodule_name}')
                    new_sub_commit = sub_repo.head.commit.hexsha
                    logger.info(f"Committed FlagScale patches in submodule. New commit: {new_sub_commit}")

                    logger.info(f"Updating main FlagScale repo to point to new submodule commit...")
                    main_repo = Repo(main_path)

                    try:
                        main_repo.config_reader().get_value('user', 'name')
                        main_repo.config_reader().get_value('user', 'email')
                    except Exception:
                        logger.warning("Git user info not set. Using default bot for this local commit.")
                        with main_repo.config_writer() as writer:
                            writer.set_value('user', 'name', 'flagscale-bot')
                            writer.set_value('user', 'email', 'bot@flagscale.com')

                    main_repo.git.add(dst)
                    main_repo.git.commit('-m', f'Update {submodule_name} with FlagScale base patches')
                    logger.info(f"Main repo commit created. HEAD is now at {main_repo.head.commit.hexsha}")

                else:
                    logger.info(f"No FlagScale patches to commit in submodule {submodule_name} (clean).")

            except Exception as e:
                logger.error(f"Failed to auto-commit submodule changes: {e}", exc_info=True)
                raise e
            # ==================================================================
