...
```sh
cd FlagScale/
vim examples/pi0_lerobot/conf/train/pi0.yaml
```
...
```sh
cd FlagScale/
python run.py --config-path ./examples/pi0_lerobot/conf --config-name train action=run
```
...
```sh
cd FlagScale/
vim examples/pi0_lerobot/conf/inference/pi0.yaml
```
...
cd FlagScale/
python run.py \
    --config-path ./examples/pi0_lerobot/conf \
...
```sh
cd FlagScale/
vim examples/pi0_lerobot/conf/serve/pi0.yaml
```
...
```sh
cd FlagScale/
python run.py --config-path ./examples/pi0_lerobot/conf --config-name serve action=run
```
