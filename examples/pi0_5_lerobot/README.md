...
mkdir -p path-to-your-pi05_base-model
cd path-to-your-pi05_base-model
...
```sh
modelscope download --model lerobot/pi05_base --local_dir path-to-your-pi05_base-model
...
```sh
cd FlagScale/
vim examples/pi0_5_lerobot/conf/train/pi0_5.yaml
```
...
- `model.checkpoint_dir` -> Path to pretrained model (e.g., `path-to-your-pi05_base-model`)
...
- `data.rename_map` -> JSON string mapping dataset keys to policy keys (optional):
  ...
  ```
- `data.use_quantiles` -> Set to `false` to use MEAN_STD normalization for `pi0.5` (default: `false`)

## Start Training

```sh
cd FlagScale/
python run.py --config-path ./examples/pi0_5_lerobot/conf --config-name train action=run
```
...
```sh
cd FlagScale/
python examples/pi0_lerobot/dump_dataset_inputs.py \
...
```sh
cd FlagScale/
vim examples/pi0_5_lerobot/conf/inference/pi0_5.yaml
```
...
- `engine.model` -> Path to pretrained model (e.g., `path-to-your-pi05_base-model`)
...
- `engine.use_quantiles` -> Set to `false` to use MEAN_STD normalization for `pi0.5` (default: `false`)
...
cd FlagScale/
python run.py \
    --config-path ./examples/pi0_5_lerobot/conf \
...
```sh
cd FlagScale/
vim examples/pi0_5_lerobot/conf/serve/pi0_5.yaml
```
...
```sh
cd FlagScale/
python run.py --config-path ./examples/pi0_5_lerobot/conf --config-name serve action=run
```
